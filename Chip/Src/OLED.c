...
{ 
     ...
     gpio_init(LCD_SCL_Pin,GPO,1);
     gpio_init(LCD_SDA_Pin,GPO,1);
     gpio_init(LCD_RST_Pin,GPO,1);
     gpio_init(LCD_DC_Pin,GPO,1);
  ...
  Set_Display_On_Off(0x00);		  // Display Off (0x00/0x01)            LCD_WrCmd(0xAE|d);
...
//输出汉字和字符混合字符串
void LCD_Print(byte x, byte y, byte ch[])   //在第x列，y行显示
{
	byte ch2[3];
	byte ii=0;        
	while(ch[ii] != '\0')
...
void LCD_ShowLine(void)
{
   ...
   for(i=0;i<60;i++)
   {
   	 for(j=0;j<80;j++)
   	 {
		if(Road_Left[i]==j) img[i][j+2]=0;
		else if(Road_Center[i]==j) img[i][j]=0;
		else if(Road_Right[i]==j) img[i][j-2]=0;
		else img[i][j]=1;
   	 }
   ...
   { 
     /*-------------------------------------------------------------------
      Order:
              0xb0+0~7   选择页数 一页八行
              0x10+0~15  选择组数 128列 分为16组 每组8列
              0x00+0~7   选择每组的列数
                         数据RAM为竖型 高字节在下 低字节在上
      
       Author：＾AI＆THE*ANSWER＆ZQXX                                                       
...
void LCD_ShowPic(void)
{
   ...
   for(i=0;i<Valid_Cnt;i++)
   {
   	 for(j=0;j<80;j++)
   	 {
		if(Road_Left[i]==j) img[i][j+2]=1;
		else if(Road_Center[i]==j) img[i][j]=1;
		else if(Road_Right[i]==j) img[i][j-2]=1;
   	 }
